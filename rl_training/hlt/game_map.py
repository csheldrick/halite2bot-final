...
class Map:
    ...
    def nearby_entities_by_distance(self, entity, types='all'):
        """
        ...
        result = {}
        if types == 'all':
            l = self._all_ships() + self.all_planets()
        elif types == 'planet':
            l = self.all_planets()
        else:
            l = self._all_ships() 

        for foreign_entity in l:
    ...
    def obstacles_between(self, ship, target, ignore=()):
        ...
        """
        obstacles = []
        #entities = self.all_planets() + self._all_ships()
    ...
    def get_point(self, ship, angle, distance):
        new_target_dx = math.cos(math.radians(angle)) * distance
        new_target_dy = math.sin(math.radians(angle)) * distance
        return entity.Position(ship.x + new_target_dx, ship.y + new_target_dy)

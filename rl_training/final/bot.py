...
from final.model import KerasModel
import heapq
...
import time
import hlt
from final.common import *
#from final.neural_net import NeuralNet, NeuralNetOrig, NeuralNetAdam
...
class Bot:
    def __init__(self, name):
        ...
        predictions = self._kmodel.predict(random_input_data)
            #f = open('./models/conv2d-output.log', 'w')
    ...
    def play(self):
        ...
        while True:
            try:
                ...
                print(e, file=open("./models/{}-output.log".format(self._name), "w"))
    ...
    def produce_instructions(self, game_map, ships_to_planets_assignment, round_start_time):
        ...
        #d_count = 0
        me = game_map.get_me()
        #others = [s for s in game_map._all_ships() if s.owner != me]
        ...
        for ship, planet in ships_to_planets_assignment:
            ...
            if is_planet_friendly:
               if ship.can_dock(planet):
                    ...
                    #danger = []
                    #if len(me.all_ships()) < len(others) and len(me.all_planets()) < 5:
                        #danger = self.check_enemies(ship, others, game_map)
                    #if len(danger) > 0:
                        #command_queue.append(self.handle_defense(ship, danger, game_map))
                    #else:
                    command_queue.append(self.navigate(game_map, round_start_time, ship, ship.closest_point_to(planet), speed))
            else:
            ...
            #if time.time() - round_start_time < 1.2: break
    ...
    def check_enemies(self, ship, others, game_map):
        me = game_map.get_me()
        danger = [s for s in others if ship.calculate_distance_between(s) < 12]
        return danger


    def handle_defense(self, ship, others, game_map):
        # others = list from check_enemies
        me = game_map.get_me()
        eothers = others
        t = choice(eothers) #eothers.index(min(eothers, key=lambda s: np.sqrt((s.x - ship.x) ** 2 + (s.y - ship.y) ** 2)))
        #t = eothers[mi]
        speed = 7
        return ship.navigate(ship.closest_point_to(t, min_distance=1),game_map, speed=speed, max_corrections=50, angular_step=2)

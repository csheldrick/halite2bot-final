...
class Bot:
    ...
    def check_enemies(self, ship, others, game_map):
        me = game_map.get_me()
        danger = [s for s in others if ship.calculate_distance_between(s) < 15]
        return danger
    ...
    def handle_out_of_range(self, ship, planet, game_map, command_queue, angle, dist=None):
        ...
        point = ship.closest_point_to(planet, min_distance=1)
            ...
            if is_planet_friendly:
                ...
                # Not in range or In Range and no space
                    ...
                    if len(danger) > 0:
                        ...
                        command_queue.append(self.navigate(game_map, start, ship, ship.closest_point_to(planet), speed))
                    continue
            # Enemy
            else:
                # Check for enemies
                ...
                if attack_mode or len(danger) > 0:
                    ...
                    command_queue.append(
                        ship.navigate(ship.closest_point_to(weakest, min_distance=1), game_map, 7))
                    continue
                else:
                    ...
                    if dist < 3:
                        weakest = planet.weakest_ship()
                        command_queue.append(ship.navigate(ship.closest_point_to(weakest, min_distacne=1), game_map, 7))
                        continue
                    else:

...
class Planet(Entity):
    ...
    def weakest_ship(self):
        return sorted(self._docked_ships.values(), key=lambda s: s.health)[0]

    def docking_spots(self):
        return self.num_docking_spots - len(self.all_docked_ships())
...
class Ship(Entity):
    ...
    def navigate(self, target, game_map, speed, avoid_obstacles=True, max_corrections=100, angular_step=2,
    ...
    def closest_planet(self, game_map, filt=None):
        """
        ...
        if filt:
            if not type(filt) == list: filt = [filt]
            planets = [p for p in planets if p not in filt]
